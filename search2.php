...
function outputItemsGrid($result) {
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ?>
            <div class="item" data-category="<?php echo strtolower(htmlspecialchars($row['category'])); ?>">
                <div class="itemimg">
...
if (isset($_FILES['imageUpload']) && $_FILES['imageUpload']['error'] === UPLOAD_ERR_OK) {
    ...
    $sql = "SELECT * FROM image WHERE " . implode(' OR ', $sqlParts);
...
$sql = "SELECT * FROM image WHERE img_name LIKE '%$searchEscaped%' OR category LIKE '%$searchEscaped%' OR Description LIKE '%$searchEscaped%'";
$result = mysqli_query($conn, $sql);
